...
mp.events.add('createJobBuilderRoom', (type) => {
    try {
        if (type === false)
            ...
            marker = mp.markers.new(20, new mp.Vector3(-97.220, -1014.106, 27.075), 1, {
                visible: true,
                color: [255, 0, 0, 180],
                rotation: 180
            });
...
mp.events.add('create.job.builder.mark', (posx, posy, posz, type) => {
    try {
        ...
        if (type === true) {
            ...
            JobBuilder.marker = mp.markers.new(1, new mp.Vector3(posx, posy, posz - 1.3), 0.8, {
                visible: true,
                dimension: 0,
                color: [255, 0, 0, 180]
            });
...
mp.events.add('createJobNeedBuilderMarkBlip', (posx, posy, posz) => {
    try {
        deleteData();
        JobBuilder.marker = mp.markers.new(1, new mp.Vector3(posx, posy, posz - 1.4), 4, {
            visible: true,
            dimension: 0,
            color: [255, 0, 0, 110]
        });
...
mp.events.add('createJobBuilderMarkBlip', (type, type2, posx, posy, posz, rotation) => {
    try {
        deleteData();
        if (type === true) {
            JobBuilder.marker = mp.markers.new(1, new mp.Vector3(posx, posy, posz - 1.2), 1, {
                visible: true,
                dimension: 0,
                color: [255, 0, 0, 180]
            });
...
mp.events.add('create.job.builder.load', () => {
    try {
        ...
        JobBuilder.blip = mp.blips.new(1, new mp.Vector3(bonePos.x, bonePos.y), {
            alpha: 255,
            color: 1,
            name: "Пункт назначения"
        });
        JobBuilder.marker = mp.markers.new(1, new mp.Vector3(bonePos.x, bonePos.y, bonePos.z - 1.25), 1.35, {
            visible: true,
            dimension: 0,
            color: [255, 0, 0, 180]
        });
